...
const listers: Record<EntityKey, () => Promise<RawRecord[]>> = {
  'assets': () => LivingAppsService.getAssets() as Promise<RawRecord[]>,
  'teams': () => LivingAppsService.getTeams() as Promise<RawRecord[]>,
  'mitarbeitende': () => LivingAppsService.getMitarbeitende() as Promise<RawRecord[]>,
  'tickets': () => LivingAppsService.getTickets() as Promise<RawRecord[]>,
  'notizen': () => LivingAppsService.getNotizen() as Promise<RawRecord[]>,
};
...
const queriers: Record<EntityKey, (q: RecordQuery) => Promise<RawRecord[]>> = {
  'assets': q => LivingAppsService.queryAssets(q) as Promise<RawRecord[]>,
  'teams': q => LivingAppsService.queryTeams(q) as Promise<RawRecord[]>,
  'mitarbeitende': q => LivingAppsService.queryMitarbeitende(q) as Promise<RawRecord[]>,
  'tickets': q => LivingAppsService.queryTickets(q) as Promise<RawRecord[]>,
  'notizen': q => LivingAppsService.queryNotizen(q) as Promise<RawRecord[]>,
};

const counters: Record<EntityKey, (filter?: string, signal?: AbortSignal) => Promise<number>> = {
  'assets': (filter, signal) => LivingAppsService.countAssets(filter, signal),
  'teams': (filter, signal) => LivingAppsService.countTeams(filter, signal),
  'mitarbeitende': (filter, signal) => LivingAppsService.countMitarbeitende(filter, signal),
  'tickets': (filter, signal) => LivingAppsService.countTickets(filter, signal),
  'notizen': (filter, signal) => LivingAppsService.countNotizen(filter, signal),
};

const creators: Record<EntityKey, (fields: Record<string, unknown>) => Promise<RawMutation>> = {
  'assets': fields => LivingAppsService.createAsset(fields as never),
  'teams': fields => LivingAppsService.createTeam(fields as never),
  'mitarbeitende': fields => LivingAppsService.createMitarbeitendeEntry(fields as never),
  'tickets': fields => LivingAppsService.createTicket(fields as never),
  'notizen': fields => LivingAppsService.createNotizenEntry(fields as never),
};

const updaters: Record<EntityKey, (id: string, fields: Record<string, unknown>) => Promise<RawMutation>> = {
  'assets': (id, fields) => LivingAppsService.updateAsset(id, fields as never),
  'teams': (id, fields) => LivingAppsService.updateTeam(id, fields as never),
  'mitarbeitende': (id, fields) => LivingAppsService.updateMitarbeitendeEntry(id, fields as never),
  'tickets': (id, fields) => LivingAppsService.updateTicket(id, fields as never),
  'notizen': (id, fields) => LivingAppsService.updateNotizenEntry(id, fields as never),
};

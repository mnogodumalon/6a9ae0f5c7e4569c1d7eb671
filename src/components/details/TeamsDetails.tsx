...
export interface TeamsDetailsProps {
  ...
  record: Teams;
  /** Volle Liste Mitarbeitende — für N:1-Relation (lead) und 1:N (team). */
    ...
    <>
      ...
      <RecordSection title={t('relations')} cols={1}>
        <RecordRelation
          ...
          name={leadTarget ? `${leadTarget.fields.first_name ?? ''} ${leadTarget.fields.last_name ?? ''}`.trim() || '—' : '—'}
      ...
      <SatelliteSection
        ...
        map={r => ({ name: `${r.fields.first_name ?? ''} ${r.fields.last_name ?? ''}`.trim() || appLabel('mitarbeitende'), meta: undefined })}
        onOpen={(r) => onOpenMitarbeitende?.(r)}

/**
 * MitarbeitendeDialog — pre-generated create/edit dialog for Mitarbeitende.
 *
 ...
 * MitarbeitendeDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Mitarbeitende['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<MitarbeitendeDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Mitarbeitende, Teams } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Mitarbeitende';
...
/** Widened prefill type for MitarbeitendeDialog.defaultValues — see file header. */
export type MitarbeitendeDialogDefaults = Mitarbeitende['fields'];

interface MitarbeitendeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Mitarbeitende['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: MitarbeitendeDialogDefaults;
...
export function MitarbeitendeDialog({ open, onClose, onSubmit, defaultValues, recordId, teamsList, enablePhotoScan = true, enablePhotoLocation = true }: MitarbeitendeDialogProps) {
  const [fields, setFields] = useState<Partial<Mitarbeitende['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Mitarbeitende['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'mitarbeitende');
      await onSubmit(clean as Mitarbeitende['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["team"]);
        ...
        return merged as Partial<Mitarbeitende['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('mitarbeitende') })
    : t('new_entity', { entity: appLabel('mitarbeitende') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'first_name': (
      <div key="first_name" className="space-y-1.5">
        ...
        <Input
          ...
          required
        />
        {showErrors && !fields.first_name && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('mitarbeitende', 'first_name')}</p>
        )}
      </div>
    ),
    'last_name': (
      <div key="last_name" className="space-y-1.5">
        ...
        <Input
          ...
          required
        />
        {showErrors && !fields.last_name && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('mitarbeitende', 'last_name')}</p>
        )}
      </div>
    ),
    'email': (
      <div key="email" className="space-y-1.5">
        <Label htmlFor="email">{fieldLabel('mitarbeitende', 'email')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="email"
          type="email"
          inputMode="email"
          placeholder="z. B. m.schmidt@example.com"
          value={fields.email ?? ''}
          onChange={e => setFields(f => ({ ...f, email: e.target.value }))}
          required
        />
        {showErrors && !fields.email && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('mitarbeitende', 'email')}</p>
        )}
      </div>
    ),
    'phone': (
      <div key="phone" className="space-y-1.5">
        <Label htmlFor="phone">{fieldLabel('mitarbeitende', 'phone')}</Label>
        <Input
          id="phone"
          type="tel"
          inputMode="tel"
    ...
    'active': (
      <div key="active" className="space-y-1.5">
        <Label htmlFor="active">{fieldLabel('mitarbeitende', 'active')}</Label>
        <div className="flex items-center gap-2 pt-1">
          ...
          />
          <Label htmlFor="active" className="font-normal">{fieldLabel('mitarbeitende', 'active')}</Label>
        </div>
      </div>
    ),
    'working_hours_per_week': (
      <div key="working_hours_per_week" className="space-y-1.5">
        <Label htmlFor="working_hours_per_week">{fieldLabel('mitarbeitende', 'working_hours_per_week')}</Label>
        <Input
  ...
  const FIELD_LABELS: Record<string, string> = {"first_name": "Vorname", "last_name": "Nachname", "email": "E-Mail", "phone": "Telefon", "active": "Aktiv", "working_hours_per_week": "Arbeitsstunden pro Woche", "team": "Team"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"team": {"lead": "Teamleitung", "name": "Name", "cost_center": "Kostenstelle"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.MITARBEITENDE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (

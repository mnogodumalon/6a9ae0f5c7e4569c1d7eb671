/**
 * TeamsDialog — pre-generated create/edit dialog for Teams.
 *
 ...
 * TeamsDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Teams['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<TeamsDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Teams, Mitarbeitende } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Teams';
...
/** Widened prefill type for TeamsDialog.defaultValues — see file header. */
export type TeamsDialogDefaults = Teams['fields'];

interface TeamsDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Teams['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: TeamsDialogDefaults;
...
export function TeamsDialog({ open, onClose, onSubmit, defaultValues, recordId, mitarbeitendeList, enablePhotoScan = true, enablePhotoLocation = true }: TeamsDialogProps) {
  const [fields, setFields] = useState<Partial<Teams['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Teams['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'teams');
      await onSubmit(clean as Teams['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "lead": string | null, // Display name from Mitarbeitende (see <available-records>)\n  "name": string | null, // Name\n  "cost_center": string | null, // Kostenstelle\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["lead"]);
        ...
        if (leadName) {
          const leadMatch = mitarbeitendeList.find(r => matchName(leadName!, [[r.fields.first_name ?? '', r.fields.last_name ?? ''].filter(Boolean).join(' ')]));
          if (leadMatch) merged['lead'] = createRecordUrl(APP_IDS.MITARBEITENDE, leadMatch.record_id);
        }
        return merged as Partial<Teams['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('teams') })
    : t('new_entity', { entity: appLabel('teams') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'lead': (
      <div key="lead" className="space-y-1.5">
        ...
        <Combobox
          id="lead"
          placeholder="Teamleiter/in wählen"
          items={mitarbeitendeListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.first_name ?? r.record_id),
          }))}
          value={extractRecordId(fields.lead)}
          onChange={id => setFields(f => ({ ...f, lead: id ? createRecordUrl(APP_IDS.MITARBEITENDE, id) : undefined }))}
          onCreateNew={(q) => openCreateMitarbeitende("lead", q)}
    ...
    'name': (
      <div key="name" className="space-y-1.5">
        ...
        <Input
          id="name"
          placeholder="z. B. Support-Team Ost"
          value={fields.name ?? ''}
          onChange={e => setFields(f => ({ ...f, name: e.target.value }))}
          required
        />
        {showErrors && !fields.name && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('teams', 'name')}</p>
        )}
      </div>
    ),
    'cost_center': (
      <div key="cost_center" className="space-y-1.5">
        <Label htmlFor="cost_center">{fieldLabel('teams', 'cost_center')}</Label>
        <Input
          id="cost_center"
          placeholder="z. B. CC-2024-501"
  ...
  const FIELD_LABELS: Record<string, string> = {"lead": "Teamleitung", "name": "Name", "cost_center": "Kostenstelle"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"lead": {"first_name": "Vorname", "last_name": "Nachname", "email": "E-Mail", "phone": "Telefon", "active": "Aktiv", "working_hours_per_week": "Arbeitsstunden pro Woche", "team": "Team"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.TEAMS} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (

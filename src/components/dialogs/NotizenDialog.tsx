...
import { TicketsDialog } from '@/components/dialogs/TicketsDialog';
...
export type NotizenDialogDefaults = Notizen['fields'];
...
export function NotizenDialog({ open, onClose, onSubmit, defaultValues, recordId, ticketsList, enablePhotoScan = true, enablePhotoLocation = true }: NotizenDialogProps) {
  ...
  // Inline-Create state for "Tickets" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraTickets` list, and select it in
  // the originating Combobox via the captured `createTicketsField`.
  const [createTicketsOpen, setCreateTicketsOpen] = useState(false);
  const [createTicketsInitial, setCreateTicketsInitial] = useState('');
  const [createTicketsField, setCreateTicketsField] = useState<string>('');
  const [extraTickets, setExtraTickets] = useState< Tickets[]>([]);
  const ticketsListAll = useMemo(
    () => [...ticketsList, ...extraTickets],
    [ticketsList, extraTickets],
  );
  function openCreateTickets(fieldKey: string, q: string) {
    setCreateTicketsField(fieldKey);
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "ticket": string | null, // Display name from Tickets (see <available-records>)\n  "text": string | null, // Text\n  "author": string | null, // Verfasser/in\n  "visible_to_reporter": boolean | null, // Für meldende Person sichtbar\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["ticket"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const ticketName = raw['ticket'] as string | null;
        if (ticketName) {
          const ticketMatch = ticketsList.find(r => matchName(ticketName!, [String(r.fields.title ?? '')]));
          if (ticketMatch) merged['ticket'] = createRecordUrl(APP_IDS.TICKETS, ticketMatch.record_id);
        }
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'ticket': (
      <div key="ticket" className="space-y-1.5">
        <Label htmlFor="ticket">{fieldLabel('notizen', 'ticket')}</Label>
        <Combobox
          id="ticket"
          placeholder="Zu welchem Ticket gehört die Notiz?"
          items={ticketsListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.title ?? r.record_id),
          }))}
          value={extractRecordId(fields.ticket)}
          onChange={id => setFields(f => ({ ...f, ticket: id ? createRecordUrl(APP_IDS.TICKETS, id) : undefined }))}
          onCreateNew={(q) => openCreateTickets("ticket", q)}
    ...
    'text': (
      <div key="text" className="space-y-1.5">
        ...
        <Textarea
          id="text"
          placeholder="Notiz eintragen, Lösungsschritte, Beobachtungen..."
          value={fields.text ?? ''}
    ...
    'visible_to_reporter': (
      <div key="visible_to_reporter" className="space-y-1.5">
        <Label htmlFor="visible_to_reporter">{fieldLabel('notizen', 'visible_to_reporter')}</Label>
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="visible_to_reporter"
            checked={!!fields.visible_to_reporter}
            onCheckedChange={(v) => setFields(f => ({ ...f, visible_to_reporter: !!v }))}
          />
          <Label htmlFor="visible_to_reporter" className="font-normal">{fieldLabel('notizen', 'visible_to_reporter')}</Label>
        </div>
      </div>
    ),
  };
  ...
  return (
    ...
    {createTicketsOpen && (
      <TicketsDialog
        ...
        onClose={() => setCreateTicketsOpen(false)}
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createTicket(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as Tickets;
            setExtraTickets(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.TICKETS, result.id);
            setFields(prev => ({ ...prev, [createTicketsField]: url } as any));
          }
          setCreateTicketsOpen(false);
        }}
        defaultValues={createTicketsInitial
          ? ({ title: createTicketsInitial } as any)
          : undefined}
        mitarbeitendeList={[]}
        teamsList={[]}

/**
 * AssetsDialog — pre-generated create/edit dialog for Assets.
 *
 ...
 * AssetsDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Assets['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<AssetsDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Assets';
...
/** Widened prefill type for AssetsDialog.defaultValues — see file header. */
export type AssetsDialogDefaults = Assets['fields'];
...
interface AssetsDialogProps {
  ...
  defaultValues?: AssetsDialogDefaults;
...
export function AssetsDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: AssetsDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Assets['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'assets');
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      if (gps) {
        setFields(f => ({ ...f, location: { lat: gps.latitude, long: gps.longitude, info: geoAddr } as any }));
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('assets') })
    : t('new_entity', { entity: appLabel('assets') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'name': (
      <div key="name" className="space-y-1.5">
        ...
        {showErrors && !fields.name && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('assets', 'name')}</p>
        )}
      </div>
    ),
    'serial_number': (
      <div key="serial_number" className="space-y-1.5">
        <Label htmlFor="serial_number">{fieldLabel('assets', 'serial_number')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="serial_number"
          placeholder="z. B. DEL-2024-789456"
          value={fields.serial_number ?? ''}
          onChange={e => setFields(f => ({ ...f, serial_number: e.target.value }))}
          required
        />
        {showErrors && !fields.serial_number && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('assets', 'serial_number')}</p>
        )}
      </div>
    ),
    'location': (
      <div key="location" className="space-y-1.5">
        <Label htmlFor="location">{fieldLabel('assets', 'location')}</Label>
        <div className="space-y-3">
          <Button type="button" variant="outline" className="w-full max-sm:h-11" disabled={locating} onClick={() => geoLocate("location")}>
          ...
          <AddressAutocomplete
            ...
            onSelect={r => setFields(f => ({ ...f, location: { lat: r.lat, long: r.long, info: r.label } as any }))}
          />
          {geoFromPhoto && fields.location && (
          ...
          {fields.location?.lat != null && fields.location?.long != null && (
            <GeoMapPicker
              lat={fields.location.lat}
              lng={fields.location.long}
              onChange={(lat, lng) => handleMapMove("location", lat, lng)}
            />
          )}
              ...
              <div>
                ...
                <Input type="number" step="any"
                  ...
                  onChange={e => {
                    ...
                    setFields(f => ({ ...f, location: { ...(f.location as any ?? {}), lat: v ? Number(v) : undefined } }));
                  }}
                />
              </div>
              <div>
                ...
                <Input type="number" step="any"
                  value={fields.location?.long ?? ''}
                  onChange={e => {
                    const v = e.target.value;
                    setFields(f => ({ ...f, location: { ...(f.location as any ?? {}), long: v ? Number(v) : undefined } }));
                  }}
                />
              </div>
            </div>
    ...
    'purchased_on': (
      <div key="purchased_on" className="space-y-1.5">
    ...
    'warranty_until': (
      <div key="warranty_until" className="space-y-1.5">
        <Label htmlFor="warranty_until">{fieldLabel('assets', 'warranty_until')}</Label>
        <DatePicker
          id="warranty_until"
          placeholder="Bis wann gilt die Garantie?"
          mode="date"
  ...
  const FIELD_LABELS: Record<string, string> = {"name": "Name", "serial_number": "Seriennummer", "location": "Standort", "purchased_on": "Angeschafft am", "warranty_until": "Garantie bis"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.ASSETS} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
